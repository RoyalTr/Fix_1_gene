...
def simulate_population(Ni, r, K, s_A, p_A_i, generations, attempts, h_A):
    ...
    for attempt_idx in range(1, attempts + 1):
        ...
        p_A_fix = 1 - (1 / (2 * N))  # Fixation threshold
        p_A_t = p_A_i  # Reset A-allele proportion
        
        # Pre-calculate Beverton-Holt constants
        r_plus_1 = 1.0 + r
        r_div_K = r / K

        for gen in range(generations):
            # The logic data represents the population state at the START of generation 'gen',
            ...
            if p_A_t == 0.0:  # a-allele fixed
                ...
                sum_N_a_final += N
                break
            elif p_A_t > p_A_fix:  # A-allele fixed
                A_count += 1
                fixation_gen = max(0, gen - 1)
                sum_A_fix_gens += fixation_gen
                sum_A_fix_gens_sq += fixation_gen * fixation_gen
